...
def bulking():
    ...
    #PASSAR O CONTEUDO DO BANCO DE DADOS DAS GKEYS PARA A LISTA PARA PODER USALOS EM UM LOOP PARA IR CRIANDO VARIAVEIS NA LISTA PARA FUTURAMENTE NAO PRECISAR CRIAR, E SER CRIADA SOZINHA, E DEPOIS CRIAR UM LAYOUT E DEIXAR NO BANCO DE DADOS PARA CADA VEZ QUE O BOTAO SEJA ACIONADO, SEJA CRIADO UMA NOVA AREA PARA SETAR VALOR, E SEPARAR DE UMA FORMA A ORGANIZAR
...
def cutting():
    ...
    for a in range(len(list1)):
        for k in range(len(list)):
            if a==0:
                carb += list3[a]*eval(list1[a].get(list[k]))
            elif a==1:
                protl += list3[a]*eval(list1[a].get(list[k]))
            elif a==2:
                proth += list3[a]*eval(list1[a].get(list[k]))
            elif a==3:
                fat += list3[a]*eval(list1[a].get(list[k]))
    pesoGordo = pesoTotal*bodyFat
    pesoMagro = pesoTotal-pesoGordo
    harrisBennedictMale = 66.5+(13.75*pesoMagro)+(5.003*altura)-(6.755*idade)
    if pesoTotal==0:
    ...
    while xCut<6:
        if list2Cutting[xCut]>list1Cutting[xCut] and xCut==2: #carb
            print("Está faltando %.2f de carboidrato, no cutting\n"%(list2Cutting[xCut]-list1Cutting[xCut])+"Sugestão de carbo:\n{:.2f} gramas de arroz\n{:.2f} gramas de testecarb\n{:.2f}gramas de udon\n{:.2f} gramas de banana".format((carbCutRestante/(list1[6].get("carb"))),(carbCutRestante/(list1[34].get("carb"))),(carbCutRestante/(list1[2].get("carb"))),(carbCutRestante/(list1[4].get("carb"))))+"\n")
            if list2Cutting[4]<list1Cutting[4]:
                pass
            else:
                print("Sugestão de carbo ENGLOBANDO A GORDURA:\n{:.2f} gramas de arroz\n{:.2f} gramas de testecarb\n{:.2f}gramas de udon\n{:.2f} gramas de banana".format(((carbCutRestante+(fatRestante*9/4))/(list1[6].get("carb"))),((carbCutRestante+(fatRestante*9/4))/(list1[34].get("carb"))),((carbCutRestante+(fatRestante*9/4))/(list1[2].get("carb"))),((carbCutRestante+(fatRestante*9/4))/(list1[4].get("carb")))))
            ...
            print("Está faltando %.2f de proteína de alto valor biológico, no cuutting\n"%(list2Cutting[xCut]-list1Cutting[xCut])+"Sugestão de proteína de alto valor biológico:\n{:.2f} gramas de frango\n{:.2f} gramas de testeprot\n{:.2f}gramas de bife\n{:.2f}gramas de wgrowth\n{:.2f}gramas de ovo cozido\n{:.2f}gramas de ovo frito\n{:.2f}gramas de leite".format((prothCutRestante/(list1[5].get("proth"))),(prothCutRestante/(list1[33].get("proth"))),(prothCutRestante/(list1[8].get("proth"))),(prothCutRestante/(list1[22].get("proth"))),(prothCutRestante/(list1[3].get("proth"))),(prothCutRestante/(list1[12].get("proth"))),(prothCutRestante/(list1[0].get("proth")))))
